...
class CXml
{
    ...
    public function render($cxmlVersion= 'http://xml.cXML.org/schemas/cXML/1.2.054/cXML.dtd', $deploymentMode = '') : string
    {
        $xmlData = <<<XML
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE cXML SYSTEM "$cxmlVersion">
...
XML;
        ...
        if (!empty($this->requests)) {
            ...
            $requestChild = $xml->addChild('Request');
            if (!empty($deploymentMode)) {
                $requestChild->addAttribute('deploymentMode',$deploymentMode);
            }    

            foreach ($this->requests as $request) {
                $request->render($requestChild);
            }
        }
